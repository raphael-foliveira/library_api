...
def test_find_book(setup):
    response = client.get(f"/books/{books_stub[0].id}")
    assert response.status_code == 200


def test_find_non_existing_book(setup):
...
def test_create_invalid_book(setup):
...
def test_delete_non_existing_book(setup):
    non_existing_book_id = 650
...
def test_get_upload_path(setup):
